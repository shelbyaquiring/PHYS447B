...
print(f'Angle between Planes: {plane_angle * 180/np.pi} [deg]')
...
endidx_0 = startidx_0 #+ 61 # Initial end date
span = 1000
step = 10
# step = 500
...
fig = plt.figure()
ax = fig.add_subplot(projection='3d')
indices = deltaV*1e-3 < 15
ax.plot_surface(X, Y, deltaV*1e-3, alpha=0.6, cmap='cool')

fig, ax = plt.subplots()
p = ax.scatter(X[indices], Y[indices], c=deltaV[indices]*1e-3, cmap='cool')

X_flat = [x for y in X for x in y]
# Y_flat = [x for y in Y for x in y]
dV_flat = [x for y in deltaV for x in y]

# min_index = np.where(dV_flat == np.min(dV_flat))

deltaV_clean = np.array([x for y in np.transpose(deltaV) for x in y if not np.isnan(x)])

cutoffs = [0, 200, 800, 1600, 1800]
min_indices = []
for idx, c in enumerate(cutoffs[:-1]):
    if idx == 0:
        label='Minimum dV'
    else:
        label=''
    # xrange_index = [np.where(X_flat == x) for x in X_flat if x > cutoffs[idx] and x < cutoffs[idx+1]]
    # deltaV_clean = np.array([x for y in np.transpose(dV_flat[xrange_index]) for x in y if not np.isnan(x)])
    min_index = deltaV == np.nanmin(deltaV_clean)

    opt = ax.scatter(X[min_index], Y[min_index], s=50, color='red', label=label)
...
gamma_e = np.arccos(np.dot(V_inf_E, [1, 0, 0]) / np.linalg.norm(V_inf_E))
gamma_v = np.arccos(np.dot(V_inf_V, [1, 0, 0]) / np.linalg.norm(V_inf_V))

print(f'gamma_e: {gamma_e*180 / np.pi} [deg]')
print(f'gamma_v: {gamma_v*180 / np.pi} [deg]')
...
# print(f'r2: {r2_ret} [m]')
# print(f'R2: {R2} [m]')
print(f'v1: {v1_pro} [m/s]')
print(f'V1: {VE[0]} [m/s]')
print(f'v2: {v2_pro} [m/s]')
print(f'V2: {VV[1]} [m/s]')
print(f'V_infty_E: {V_inf_E} [m/s]')
print(f'V_infty_V: {V_inf_V} [m/s]')
...
fig, ax = plt.subplots()
# ax.quiver(*[x / np.linalg.norm(VE[0][:2]) for x in VE[0][:2]])
sc = 100000
ax.quiver(0, 0, *VE[0][:2], color='red', alpha=0.5, scale=sc, angles='uv', label='VE')
ax.quiver(0, 0, *VV[1][:2], color='green', alpha=0.5, scale=sc, angles='uv', label='VV')
ax.quiver(0, 0, *v1_pro[:2], color='blue', alpha=0.5, scale=sc, angles='uv', label='v1')
ax.quiver(0, 0, *v2_pro[:2], color='orange', alpha=0.5, scale=sc, angles='uv', label='v2')
ax.quiver(0, 0, *V_inf_E[:2], color='purple', alpha=0.5, scale=sc, angles='uv', label='v_inf_E')
ax.quiver(0, 0, *V_inf_V[:2], color='black', alpha=0.5, scale=sc, angles='uv', label='v_inf_V')
# ax.set_xlim([-3500000000, 3500000000])
# ax.set_ylim([-3500000000, 3500000000])
...
print(f'vc_e = {vc_e}')
print(f'vc_v = {vc_v}')
print(f'V_inf_E = {np.linalg.norm(V_inf_E)}')
print(f'V_inf_V = {np.linalg.norm(V_inf_V)}')
print(f'V_inf_tot = {np.linalg.norm(V_inf_E) + np.linalg.norm(V_inf_V)}')
...
# Theoretical dV at apoapse
dV_apo = 2*np.linalg.norm(v_before)*np.sin(plane_angle/2)#np.sqrt(2 * (1 - np.cos(plane_angle))) * np.linalg.norm(v_before)
print(f'dV_apo: {dV_apo}')
...
mo = mf * np.exp(dV_total_3 / (g0 * Isp))
print(f'Initial Mass: {mo} [kg]')
print(f'Propelant Mass Fraction: {(mo - mf)/mo * 100} %')
