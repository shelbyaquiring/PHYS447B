...
def numericalSolution(initial_state, mu, reltol, T, solver, oblate=False, drag=False):
    ...
    def state_derivative(time, state):
        ...
        if not np.linalg.norm(r) > 10:#Re: # If orbit is below the surface of the earth, no need to solve
            v = [0, 0, 0]
            a = [0, 0, 0]
...
def angles(R1, R2, R3, rho1_hat, rho2_hat, rho3_hat, t1, t2, t3, format='sv'):
    """
    ...
    # xvec = np.linspace(-1000000, 1000000, 10000000)*10
    # yvec = np.polyval([1, 0, a, 0, 0, b, 0, 0, c], xvec)
...
if __name__ == '__main__':
    ...
    # # Extract 2 positions
    # latitude = 10 * np.pi / 180
    # R_observation = np.array([np.cos(np.pi/4), np.cos(np.pi/4), 0]) * Re
    # idx1 = 100
    # idx2 = 200
    # idx3 = 300
    ...
    # t3 = thetavec[idx3]
    # time = [t1, t2, t3]
    # offset = np.array([0, 0, 1e8])
    # rhohat_1 = r1 - R_observation + offset
    # rhohat_1 /= np.linalg.norm(rhohat_1)#+ [0, 0, 1]/ np.linalg.norm(r1 - R_observation + [0, 0, 1])
    # rhohat_2 = r2 - R_observation + offset#/ np.linalg.norm(r2 - R_observation)
    # rhohat_2 /= np.linalg.norm(rhohat_2)
    # rhohat_3 = r3 - R_observation#/ np.linalg.norm(r3 - R_observation)
    # rhohat_3 /= np.linalg.norm(rhohat_3)
    ...
    for idx, index in enumerate(indices):
        ...
        # R[idx,0] = (fac1 + H)*np.cos(phi)*np.cos(theta[idx])
        # R[idx,1] = (fac1 + H)*np.cos(phi)*np.sin(theta[idx])
        # R[idx,2] = fac2
        # rho[idx,0] = np.cos(dec[idx])*np.cos(ra[idx])
        # rho[idx,1] = np.cos(dec[idx])*np.sin(ra[idx])
        # rho[idx,2] = np.sin(dec[idx]) 
    ...
    # Estimate orbit
    r2, v2 = angles(R[0], R[1], R[2], rho[0], rho[1], rho[2], t[0], t[1], t[2])
    # r2, v2 = angles( R[0, :], R[1, :], R[2, :], rho[0, :], rho[1, :], rho[2, :], t[0], t[1], t[2])

    s_est = [*r2, *v2]
    # s_est = [*r2 + Re * np.array(r2) / np.linalg.norm(r2), *v2]
    orbit_est = numericalSolution(s_est, mu, reltol, T, solver)

    solvec = [orbit_n, orbit_est]

    fig, ax, _ = plot_3D_orbits(solvec, Re)
    ax.scatter3D(X, Y, Z, 'o', label='Measurements')
    ax.legend()
    plt.show()
